...
async function fixFacultyFinal() {
    ...
    try {
        const email = "harishp.mz23@bitsathy.ac.in";
        ...
        if (userRes.rows.length === 0) {
            console.log("❌ User NOT FOUND in SELECT query!");
            // Check for potential whitespace match?
            const fuzzyRes = await client.query(`SELECT normalized_email FROM users WHERE normalized_email LIKE '%harishp%'`);
            console.log("Did you mean one of these?", fuzzyRes.rows);
            process.exit(1);
        }
        ...
        if (personRes.rows.length === 0) {
            console.log("❌ Person profile NOT FOUND. Creating one...");
            // Auto-create person if missing?
            // For now, fail but log. User likely has person if they logged in.
            process.exit(1);
        }
